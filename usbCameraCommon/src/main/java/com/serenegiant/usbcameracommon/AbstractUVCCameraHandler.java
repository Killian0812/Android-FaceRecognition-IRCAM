...
abstract class AbstractUVCCameraHandler extends Handler {
	...
	private static final int MSG_CHANGE_PREVIEWSETTING = 10;
	...
	protected  void changePreviewSetting()
	{
		Log.d("CameraThread", "AbstractUVCCameraHandler mShowCamera: " + mShowCamera);
		sendEmptyMessage(MSG_CHANGE_PREVIEWSETTING);
	}
	...
	public void setFrameCallback(final IFrameCallback callback, final int pixelFormat)
	{
		...
		if (!mReleased && (callback != null)) {
			final CameraThread thread = mWeakThread.get();
			if (thread != null) {
				thread.mUVCCamera.setFrameCallback(callback, pixelFormat);
			}
		}
	}
	...
	public void handleMessage(final Message msg) {
		...
		case MSG_PREVIEW_START:
			thread.mShowCamera = mShowCamera;
		...
		case MSG_RELEASE:
			...
			break;
		case MSG_CHANGE_PREVIEWSETTING:
			thread.mShowCamera = mShowCamera;
			thread.handleChangePreviewSetting();
			break;
	...
	static final class CameraThread extends Thread {
		...
		public void handleStartPreview(final Object surface) {
			...
			mUVCCamera.mShowCamera = mShowCamera;
		...
		public void handleChangePreviewSetting() {
			if (DEBUG) Log.v(TAG_THREAD, "handleChangePreviewSetting");
			Log.d(TAG_THREAD, "Abstract mShowCamera = " + mShowCamera);
			mUVCCamera.mShowCamera = mShowCamera;
			mUVCCamera.changePreviewSetting();
		}
